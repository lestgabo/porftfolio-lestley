...
import { Button, Drawer, List, ListItem, Box, Divider, makeStyles, Typography } from '@material-ui/core';
import { SlimToolbar } from '@tacogator/gatsby-theme-blog-material-clarisse/src/components/bear/BearAppBar';
...
export function DesktopMenu(props) {
    ...
    return (
        <SlimToolbar disableGutters className={useStyles().desktopMenuItem}>
            {Array.isArray(navigation) &&
                navigation.map(({ slug, title }) => (
                    <Button key={slug} size="small" component={Link} to={replaceSlashes(slug)}>
                        {title.toUpperCase()}
                    </Button>
                ))}
        </SlimToolbar>
